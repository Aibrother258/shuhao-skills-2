...
- **开场钩子 + 结尾悬念** — 每集都要落在纸面；认领的大纲爽点必须有戏扛
...
- **台词本**：**一排两个**，按角色聚合全部台词，列表最多 6 行高可滚动，带集/场引用和「复制全部台词」——TTS 是按角色批量跑的，这页就是工作清单
...
node scripts/novel-script.mjs render script.json --html \
     --outline outline.json --art art.json > script-report.html  # 出报告
...
examples/
  渡口-script.json        《渡口》全 6 集完整剧本，全部质量门通过，也是自测夹具
assets/
  report.webp            报告截图
```
